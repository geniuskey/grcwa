...
![전파 vs 소산 모드](../assets/images/propagating_evanescent.png)

*전파 모드 vs 소산 모드: 왼쪽은 실수 $k_z$를 가진 전파 모드로 진동하며 에너지를 운반합니다. 오른쪽은 허수 $k_z$를 가진 소산 모드로 지수적으로 감쇠하며 에너지를 운반하지 않습니다.*
...
![경계 조건](../assets/images/boundary_conditions.png)

*경계 조건: 왼쪽 그림은 접선 전기장 성분이 경계면을 가로질러 연속임을 보여줍니다. 오른쪽 그림은 법선 전기장 성분이 불연속이지만 변위장 $\mathbf{D}$는 연속임을 보여줍니다.*
...
![고유모드 전파](../assets/images/eigenmode_concept.png)

*고유모드 전파: 패턴 레이어에서의 고유모드 전파를 보여줍니다. 전방 전파 모드(파랑)와 후방 전파 모드(빨강)가 여러 레이어를 통과합니다. 각 레이어는 고유한 유전 프로파일을 가지며, 모드는 경계 조건에 의해 결합됩니다.*
...
![Fourier 급수 수렴](../assets/images/fourier_expansion.png)

*Fourier 급수 수렴: 정사각파를 근사하는 Fourier 급수가 항의 개수에 따라 어떻게 수렴하는지 보여줍니다. 1항(빨강)은 매우 조잡한 근사이지만, 3항(초록), 7항(파랑), 15항(보라)으로 증가하면 원래 파형(검정)에 점점 더 가까워집니다. 날카로운 모서리에서의 진동(Gibbs 현상)은 Fourier 절단의 특징입니다.*
...
![수렴 테스트](../assets/images/convergence_test.png)

*수렴 테스트: 왼쪽 그림은 Fourier 차수 수($N_G$)에 따른 반사율(R)과 투과율(T)의 수렴을 보여줍니다. $R + T \approx 1$은 에너지 보존을 확인합니다. 오른쪽 그림은 $N_G$에 따른 계산 비용의 증가를 보여줍니다 (약 $O(N_G^{2.5})$ 스케일링).*
