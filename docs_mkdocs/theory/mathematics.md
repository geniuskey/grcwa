...
![S-행렬 개념](../assets/images/smatrix_concept.png)

*S-행렬 공식화: RCWA 구조에서 입사파, 반사파, 투과파 간의 관계를 보여줍니다. 왼쪽에서 들어오는 파 $a^+$는 부분적으로 반사($b^-$)되고 부분적으로 투과($b^+$)됩니다. 오른쪽에서 들어오는 파 $a^-$도 마찬가지입니다. S-행렬은 이러한 산란 과정을 수학적으로 표현합니다: $S_{11}$은 왼쪽에서의 반사, $S_{21}$은 왼쪽→오른쪽 투과, $S_{12}$는 오른쪽→왼쪽 투과, $S_{22}$는 오른쪽에서의 반사를 나타냅니다.*
